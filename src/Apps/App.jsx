...
export default function App() {
  ...
  // ポーリング：名簿編集画面表示中は5秒ごとにKVから最新名簿を取得
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        if (Array.isArray(latestMembers) && latestMembers.length > 0) {
          // 現在のuserListと比較して変更があれば更新
          const currentNames = userList.filter(u => !u.removed && u.name).map(u => u.name);
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          // 有効なメンバー（除籍されていない人）の名前リストを抽出
                          const activeMembers = userList
                            .filter(u => !u.removed && u.name.trim())
                            .map(u => u.name.trim());

                          // KVに名簿を保存
                          await updateTeamMembers({ team: teamName, members: activeMembers });

                          setGateOpen(false);
                        } catch (err) {
                          console.error("KV名簿保存エラー:", err);
                          alert("名簿の保存に失敗しました。ネットワーク接続を確認してください。");
                        }
                      }}
                    >
                      名簿を保存
